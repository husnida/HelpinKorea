...
async def send_welcome(message: types.Message):
    await bot.send_message(message.from_user.id, matnlar1.welcome_text, reply_markup=buttons.welcome_keyboard, parse_mode = ParseMode.MARKDOWN)
...
async def culture_or_live(message: types.Message):
    ...
    if text ==  "Koreada yashash":
        await bot.send_message(user_id, matnlar1.help_text, reply_markup = buttons.live, parse_mode = ParseMode.MARKDOWN)
    else:
        await bot.send_message(user_id, matnlar1.culture_text,reply_markup = buttons.culture, parse_mode = ParseMode.MARKDOWN)
...
async def about_live(message: types.Message):
    ...
    await bot.send_message(user_id, msg_txt, reply_markup = buttons.connect_to_live, parse_mode = ParseMode.MARKDOWN)
...
async def about_culture(message: types.Message):
    ...
    await bot.send_message(user_id, msg_txt, reply_markup = buttons.go_culture, parse_mode = ParseMode.MARKDOWN) 

@dp.message_handler(content_types=['text'])
async def live_Korea(message: types.Message):
    user_id = message.from_user.id
...
async def question(message: types.Message):
    user_id = question.from_user.id
    await question.answer("Biz bilan bog'langaningiz uchun sizdan minnatdormiz🤝")
